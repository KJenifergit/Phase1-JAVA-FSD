...
function displayExp(details) {
  ...
  for (i = 0; i < details.length; i++) {
    ...
    <div class="expValue" id="${details[i].id}">
      ...
      <div id="edite_delete">
        <p>
          <button id="${details[i].id}" onclick="editExpDetails(${details[i].id})"> <img src="https://thumbs.dreamstime.com/z/edit-symbol-icon-monogram-blue-colors-crystal-design-illustration-image-edit-button-symbol-icon-monogram-179562562.jpg" width="15" alt=""  /></button> 
          <button id="${details[i].id}" onclick="delExpenseDetails(${details[i].id})"><img src=https://thumbs.dreamstime.com/z/delete-icon-close-symbol-vector-no-sign-cancel-symbol-wrong-reject-illustration-delete-icon-close-symbol-vector-no-sign-cancel-131584727.jpg" width="15" alt="" /></button>
        </p>
      </div>
    </div>
  `;
